...
class BOHB(base_config_generator):
    def __init__(self, configspace, min_points_in_model=None,
        ...
        for h in hps:
            if hasattr(h, 'sequence'):
                raise RuntimeError(
                    'This version on BOHB does not support ordinal hyperparameters. Please encode %s as an integer parameter!' % (h.name))
    ...
    def get_config(self, budget):
        ...
        if sample is None:
            try:
                ...
                def minimize_me(x): return max(1e-32, g(x))/max(l(x), 1e-32)
        ...
        for i in range(array.shape[0]):
            ...
            while (np.any(nan_indices)):
                ...
                valid_indices = np.argwhere(
                    np.isfinite(array[:, nan_idx])).flatten()
    ...
    def new_result(self, job, update_model=True):
        ...
        conf = ConfigSpace.Configuration(
            self.configspace, job.kwargs["config"])
